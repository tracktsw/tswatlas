...
const SleepTrendsInsights = ({ checkIns, dailyFlareStates }: SleepTrendsInsightsProps) => {
  ...
  // Get check-ins with sleep scores for the month, including triggers
  const sleepDataWithTriggers = useMemo(() => {
    ...
    // Group by date, average sleep score and collect all triggers
    const byDate = new Map<string, { date: string; sleepScore: number; count: number; triggers: string[] }>();
    ...
    return Array.from(byDate.values())
      .map(d => ({ date: d.date, sleepScore: Math.round(d.sleepScore * 10) / 10, triggers: d.triggers }))
      .sort((a, b) => a.date.localeCompare(b.date));
  }, [checkIns, selectedMonth]);

  // Simple sleepData for chart
  const sleepData = useMemo(() => {
    return sleepDataWithTriggers.map(d => ({
      date: d.date,
